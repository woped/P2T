...
import de.dhbw.woped.process2text.contentDetermination.extraction.GatewayExtractor;
        ...
        for (RPSTNode<ControlFlow, Node> pNode : pNodeList) {
            for (RPSTNode<ControlFlow, Node> tNode : rpst.getChildren(pNode)) {
                if (tNode.getEntry() == node.getEntry()) {
                    for (de.dhbw.woped.process2text.dataModel.process.Arc arc : process.getArcs().values()) {
                        if (arc.getSource().getId() == Integer.valueOf(tNode.getEntry().getId()) && arc.getTarget().getId() == Integer.valueOf(tNode.getExit().getId())) {
                            if (arc.getLabel().toLowerCase().equals("yes")) {
                                de.dhbw.woped.process2text.dataModel.process.Activity a = process.getActivity(Integer.valueOf(tNode.getExit().getId()));
                                de.dhbw.woped.process2text.dataModel.process.Annotation anno = a.getAnnotations().get(0);
...
//			String  role = process.getGateways().get(Integer.valueOf(node.getEntry().getId())).getLane().getName();
            ...
            de.dhbw.woped.process2text.dataModel.process.Activity a = process.getActivity(Integer.valueOf(firstActivity.getExit().getId()));
    ...
    ConverterRecord convertANDSimple(RPSTNode<ControlFlow, Node> node, int activities, ArrayList<Node> conditionNodes) {
        ...
        if (arcs == 2) {
            if (conditionNodes.size() == 1) {
                de.dhbw.woped.process2text.dataModel.process.Activity a = process.getActivity(Integer.valueOf(conditionNodes.get(0).getId()));
    ...
    ConverterRecord convertEvent(de.dhbw.woped.process2text.dataModel.process.Event event) {
        ...
        switch (event.getType()) {
            ...
            // ERROR EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_ERROR:
            ...
            // TIMER EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_TIMER:
            ...
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_MSG_CAT:
                cFrag = new ConditionFragment("receive", "a message", "", "", ConditionFragment.TYPE_ONCE, new HashMap<>());
                configureFragment(cFrag);
                break;

            // ESCALATION EVENT (CATCHING)
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_ESCALATION_CAT:
            ...
            // END EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.END_EVENT:
            ...
            // ERROR EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.END_ERROR:
            ...
            case de.dhbw.woped.process2text.dataModel.process.EventType.END_SIGNAL:
            ...
            // START EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.START_MSG:
            ...
            // MESSAGE EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_MSG_THR:
                eFrag = new ExecutableFragment("send", "message", event.getLane().getName(), "");
                eFrag.bo_hasIndefArticle = true;
                return getEventSentence(eFrag);

            // ESCALATION EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_ESCALATION_THR:
                eFrag = new ExecutableFragment("trigger", "escalation", event.getLane().getName(), "");
                eFrag.bo_hasIndefArticle = true;
                return getEventSentence(eFrag);

            // LINK EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_LINK_THR:
                eFrag = new ExecutableFragment("send", "signal", event.getLane().getName(), "");
                eFrag.bo_hasIndefArticle = true;
                return getEventSentence(eFrag);

            // MULTIPLE TRIGGER
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_MULTIPLE_THR:
            ...
            // SIGNAL EVENT
            case de.dhbw.woped.process2text.dataModel.process.EventType.INTM_SIGNAL_THR:
                eFrag = new ExecutableFragment("send", "signal", event.getLane().getName(), "");
                eFrag.bo_hasArticle = true;
                eFrag.bo_hasIndefArticle = true;
                eFrag.bo_isPlural = true;
     ...
     */
    private DSynTConditionSentence getAttachedEventSentence(de.dhbw.woped.process2text.dataModel.process.Event event, ConditionFragment cFrag) {
    ...
    DSynTConditionSentence getAttachedEventPostStatement(de.dhbw.woped.process2text.dataModel.process.Event event) {
