...
public class TextPlanner {
    ...
    for (RPSTNode<ControlFlow, Node> node : orderedTopNodes) {
      ...
      if (PlanningHelper.isBond(node)) {
        ...
        if (PlanningHelper.isEvent(node.getExit())) {
          ...
          if (event.getType() == de.dhbw.woped.process2text.dataModel.process.EventType.END_EVENT
      ...
      //			 else if (PlanningHelper.isEvent(node.getEntry()) && orderedTopNodes.indexOf(node) > 0) {
        ...
        if (currentPosition == 0) {
          ...
          if (start && !isAlternative) {
            ...
            if (event.getType()
                    == de.dhbw.woped.process2text.dataModel.process.EventType.START_EVENT
  ...
  private void convertActivities(RPSTNode<ControlFlow, Node> node, int level, int depth)
    ...
    de.dhbw.woped.process2text.dataModel.process.Activity activity =
    ...
    eFrag.addAssociation(activity.getId());
    if (activity.getType()
        == de.dhbw.woped.process2text.dataModel.process.ActivityType.TYPE_MAP.get("Subprocess")) {
  ...
  private String getRole(
      de.dhbw.woped.process2text.dataModel.process.Activity a, AbstractFragment frag) {
    if (a.getLane() == null) {
