...
import de.dhbw.woped.process2text.pnmlProcessing.contentDetermination.labelAnalysis.EnglishLabelDeriver;
import de.dhbw.woped.process2text.pnmlProcessing.contentDetermination.labelAnalysis.EnglishLabelHelper;
import de.dhbw.woped.process2text.pnmlProcessing.contentDetermination.preprocessing.FormatConverter;
...
import de.dhbw.woped.process2text.pnmlProcessing.textPlanning.recordClasses.GatewayPropertyRecord;
import de.dhbw.woped.process2text.pnmlProcessing.textPlanning.recordClasses.ModifierRecord;
import de.dhbw.woped.process2text.pnmlProcessing.utils.Pair;
    ...
    public void convertToText(RPSTNode<ControlFlow, Node> root, int level) throws JWNLException, FileNotFoundException {
        ...
        for (RPSTNode<ControlFlow, Node> node : orderedTopNodes) {
            ...
            if (PlanningHelper.isBond(node)) {
                ...
                // Handle End Event
                if (PlanningHelper.isEvent(node.getExit())) {
                    de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Event event = process.getEvents().get((Integer.valueOf(node.getExit().getId())));
                    if (event.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.EventType.END_EVENT && orderedTopNodes.indexOf(node) == orderedTopNodes.size() - 1) {
...
//			 else if (PlanningHelper.isEvent(node.getEntry()) && orderedTopNodes.indexOf(node) > 0) {
                ...
                de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Event event = process.getEvents().get((Integer.valueOf(node.getEntry().getId())));
                ...
                if (currentPosition == 0) {
                    ...
                    if (start && !isAlternative) {
                        ...
                        if (event.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.EventType.START_EVENT && currentPosition < orderedTopNodes.size() - 1 && PlanningHelper.isBond(orderedTopNodes.get(currentPosition + 1))) {
                            start = false;
                        ...
                        if (event.getType() != de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.EventType.START_EVENT) {
                            start = false;
    ...
    private void convertActivities(RPSTNode<ControlFlow, Node> node, int level, int depth) throws JWNLException, FileNotFoundException {
        ...
        de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Activity activity = process.getActivity(Integer.parseInt(node.getEntry().getId()));
        de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Annotation anno = activity.getAnnotations().get(0);
        ...
        eFrag.addAssociation(activity.getId());
        if (activity.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.ActivityType.TYPE_MAP.get("Subprocess")) {
     ...
     */
    private String getRole(de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Activity a, AbstractFragment frag) {
        if (a.getLane() == null) {
