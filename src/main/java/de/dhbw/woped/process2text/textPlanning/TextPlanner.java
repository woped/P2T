...
import de.dhbw.woped.process2text.utils.Pair;
    ...
    public void convertToText(RPSTNode<ControlFlow, Node> root, int level) throws JWNLException, FileNotFoundException {
        ...
        for (RPSTNode<ControlFlow, Node> node : orderedTopNodes) {
            ...
            if (PlanningHelper.isBond(node)) {
                ...
                // Handle End Event
                if (PlanningHelper.isEvent(node.getExit())) {
                    de.dhbw.woped.process2text.dataModel.process.Event event = process.getEvents().get((Integer.valueOf(node.getExit().getId())));
                    if (event.getType() == de.dhbw.woped.process2text.dataModel.process.EventType.END_EVENT && orderedTopNodes.indexOf(node) == orderedTopNodes.size() - 1) {
...
//			 else if (PlanningHelper.isEvent(node.getEntry()) && orderedTopNodes.indexOf(node) > 0) {
                ...
                de.dhbw.woped.process2text.dataModel.process.Event event = process.getEvents().get((Integer.valueOf(node.getEntry().getId())));
                ...
                if (currentPosition == 0) {
                    ...
                    if (start && !isAlternative) {
                        ...
                        if (event.getType() == de.dhbw.woped.process2text.dataModel.process.EventType.START_EVENT && currentPosition < orderedTopNodes.size() - 1 && PlanningHelper.isBond(orderedTopNodes.get(currentPosition + 1))) {
                            start = false;
                        ...
                        if (event.getType() != de.dhbw.woped.process2text.dataModel.process.EventType.START_EVENT) {
                            start = false;
    ...
    private void convertActivities(RPSTNode<ControlFlow, Node> node, int level, int depth) throws JWNLException, FileNotFoundException {
        ...
        de.dhbw.woped.process2text.dataModel.process.Activity activity = process.getActivity(Integer.parseInt(node.getEntry().getId()));
        de.dhbw.woped.process2text.dataModel.process.Annotation anno = activity.getAnnotations().get(0);
     ...
     */
    private String getRole(de.dhbw.woped.process2text.dataModel.process.Activity a, AbstractFragment frag) {
        if (a.getLane() == null) {
