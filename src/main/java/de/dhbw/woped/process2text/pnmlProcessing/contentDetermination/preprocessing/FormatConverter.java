package de.dhbw.woped.process2text.pnmlProcessing.contentDetermination.preprocessing;

import de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Element;
import de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Activity;
...
public class FormatConverter {
	...
	public ProcessModel transformFromRigidFormat(Process p) {
		...
		for (Task t: p.getTasks()) {
			...
			if (converterMap.containsKey(id)) {
				...
				if (elem.getClass().toString().endsWith("Event")) {
					de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Event e = (de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Event) elem;
					pm.addEvent(e);
		...
		for (Gateway g: p.getGateways()) {
			if (!g.getName().equals("") && converterMap.containsKey(Integer.valueOf(g.getName()))) {
				...
				de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway gw = (de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway) converterMap.get(id);
				pm.addGateway(gw);
			} else {
				if (g.getGatewayType() == GatewayType.XOR) {
					de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.XOR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.OR) {
					de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.OR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.AND) {
					de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.AND);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.EVENT) {
					de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.EVENT);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
		...
		for (ControlFlow f: p.getControlFlow()) {
			...
			de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Arc arc = new de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Arc(getId(), f.getName(), source , target);
			pm.addArc(arc);
		}
		return pm;
	}
	...
	public Process transformToRigidFormat(ProcessModel pm) {
		...
		// Transform events
		for (de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Event e: pm.getEvents().values()) {
		...
		// Transform gateway
		for (de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Gateway g: pm.getGateways().values()) {
			if (g.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.XOR) {
				Gateway gt = new Gateway(GatewayType.XOR,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);

			}
			if (g.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.OR) {
				Gateway gt = new Gateway(GatewayType.OR,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);
			}
			if (g.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.AND) {
				Gateway gt = new Gateway(GatewayType.AND,Integer.toString(g.getId()));
				elementMap.put(g.getId(), gt);
			}
			if (g.getType() == de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.GatewayType.EVENT) {
		...
		// Transform arcs
		for (de.dhbw.woped.process2text.pnmlProcessing.dataModel.process.Arc arc: pm.getArcs().values()) {
