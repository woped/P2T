	...
	public ProcessModel transformFromRigidFormat(Process p) {
		...
		for (Task t: p.getTasks()) {
			...
			if (converterMap.containsKey(id)) {
				...
				if (elem.getClass().toString().endsWith("Event")) {
					de.dhbw.woped.process2text.dataModel.process.Event e = (de.dhbw.woped.process2text.dataModel.process.Event) elem;
					pm.addEvent(e);
		...
		for (Gateway g: p.getGateways()) {
			if (!g.getName().equals("") && converterMap.containsKey(Integer.valueOf(g.getName()))) {
				...
				de.dhbw.woped.process2text.dataModel.process.Gateway gw = (de.dhbw.woped.process2text.dataModel.process.Gateway) converterMap.get(id);
				pm.addGateway(gw);
			} else {
				if (g.getGatewayType() == GatewayType.XOR) {
					de.dhbw.woped.process2text.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.dataModel.process.GatewayType.XOR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.OR) {
					de.dhbw.woped.process2text.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.dataModel.process.GatewayType.OR);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.AND) {
					de.dhbw.woped.process2text.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.dataModel.process.GatewayType.AND);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
				if (g.getGatewayType() == GatewayType.EVENT) {
					de.dhbw.woped.process2text.dataModel.process.Gateway gw = new de.dhbw.woped.process2text.dataModel.process.Gateway(getId(), "", null, null, de.dhbw.woped.process2text.dataModel.process.GatewayType.EVENT);
					pm.addGateway(gw);
					idMap.put(g.getId(), gw.getId());
					elemMap.put(gw.getId(), gw);
				}
