package de.dhbw.woped.process2text.bpmlProcessing.contentDetermination.extraction;
...
public class GatewayExtractor {
	...
	private void processGateway(Node gateway) {
		...
		// Gateway has label	
			...
			if (s.toLowerCase().equals("all docs available")) {
				verb = "be";
				bo = "all docs";
				modList.put("available",new ModifierRecord(ModifierRecord.TYPE_ADV, ModifierRecord.TARGET_VERB));
				extracted = true;
			}
...
//				verb = "be";
			...
			if (extracted == false) {
				System.err.println("GatewayExtractor: Extraction pattern not covered: " + s);
			}
		}
	}
