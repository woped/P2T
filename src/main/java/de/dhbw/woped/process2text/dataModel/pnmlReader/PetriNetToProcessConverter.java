...
public class PetriNetToProcessConverter {
    ...
    if (!transformedElems.keySet().contains(elemId)) {
      ...
      if (elem.getClass().toString().endsWith("Place")) {
        ...
        if (petriNet.getSuccessor(elemId).size() == 0
          ...
          model.addActivity(
              new Activity(newActivityId, "complete process", null, null, ActivityType.NONE));
          ...
          model.addArc(
              new Arc(model.getNewId(), "", model.getElem(newActivityId), model.getElem(newId)));
        }
        ...
        if (petriNet.getSuccessor(elemId).size() >= 0
          ...
          if (precElem != -1) {
            model.addArc(
                new Arc(model.getNewId(), "", model.getElem(precElem), model.getElem(newId)));
          }
          ...
          if (!(petriNet.getSuccessor(elemId).size() == 0)) {
            ...
            model.addArc(
                new Arc(model.getNewId(), "", model.getElem(newId), model.getElem(newId3)));
          }
        }
        ...
        if (petriNet.getSuccessor(elemId).size() > 1
          ...
          if (checkXOR) {
            ...
            HashMap<String, de.dhbw.woped.process2text.dataModel.pnmlReader.PetriNet.Element>
                elems = petriNet.getElements();
            de.dhbw.woped.process2text.dataModel.pnmlReader.PetriNet.Element sucXOR =
                elems.get(sucId);
            ...
            if (precElem != -1) {
              ...
              if (transformedElems.keySet().contains(elemId)) {
                ...
                de.dhbw.woped.process2text.dataModel.process.Element source =
                    model.getElem(placeId);
                ArrayList<Arc> arcs = getArcWithSource(source, model);

                model.deleteArc(arcs.get(0));
                model.addArc(
                    new Arc(model.getNewId(), "", model.getElem(placeId), model.getElem(newId)));
              } else {
                model.addArc(
                    new Arc(model.getNewId(), "", model.getElem(precElem), model.getElem(newId)));
              }
            ...
            model.addArc(
                new Arc(model.getNewId(), "", model.getElem(newId), model.getElem(newId2)));
        ...
        if (petriNet.getSuccessor(elemId).size() == 1
          ...
          loopSet[x] =
              elemId
                  + ": simple transition, one incoming, one outgoing arc, Type: "
                  + elem.getType()
                  + " ,role: "
                  + elem.getRole();
          x++;
          ...
          if (elem.getType() != null && elem.getType().equals("104")) {
            ...
            model.addActivity(
                new Activity(
                    newId, xorTitle + " " + label2, roleAsLane, groupAsPool, ActivityType.NONE));
            transformedElems.put(elemId, newId);
            transformedElemsRev.put(newId, elemId);
            if (precElem != -1) {
              model.addArc(
            ...
            int newActivityId = model.getNewId();
            model.addActivity(
                new Activity(newActivityId, "finish branch", null, null, ActivityType.NONE));
            ...
            if (XORId == 0) {
              ...
              model.addArc(
                  new Arc(
                      model.getNewId(), "", model.getElem(newActivityId), model.getElem(newXorId)));

              // Activtiy erstellen welches das XOR-Join label darstellt und das mit Gateway
              // verbinden
              newActivityId = model.getNewId();
              model.addActivity(
                  new Activity(newActivityId, elem.getLabel(), null, null, ActivityType.NONE));
              transformedElems.put(elemId, newActivityId);
              transformedElemsRev.put(newActivityId, elemId);
              model.addArc(
                  new Arc(
                      model.getNewId(), "", model.getElem(newXorId), model.getElem(newActivityId)));
            ...
            for (String suc : petriNet.getSuccessor(elemId)) {
              transformElem(
                  petriNet.getElements().get(suc), activityId, petriNet, model, pool, lane);
            }
            ...
            int type =
                elem.getId().contains("sub")
                    ? ActivityType.TYPE_MAP.get("Subprocess")
                    : ActivityType.NONE;
        ...
        if (petriNet.getSuccessor(elemId).size() == 1
          ...
          if (!(joinSplitWords.contains(label) || label.matches("([a-z]+)\\d+") || label.equals(""))
            ...
            String suc = petriNet.getSuccessor(elemId).get(0);
            transformElem(
                petriNet.getElements().get(suc), newActivityId, petriNet, model, pool, lane);
          } else {
            // Recursively go through the model
            String suc = petriNet.getSuccessor(elemId).get(0);
            transformElem(petriNet.getElements().get(suc), newId, petriNet, model, pool, lane);
      ...
      model.addArc(
          new Arc(
              model.getNewId(),
              "",
              model.getElem(precElem),
              model.getElem(transformedElems.get(elem.getId()))));
    }
  }
  ...
  private ArrayList<Arc> getArcWithSource(
      de.dhbw.woped.process2text.dataModel.process.Element element, ProcessModel model) {
