package de.dhbw.woped.process2text.contentDetermination.extraction;
...
public class GatewayExtractor {
  ...
  private void processGateway(Node gateway) {
    ...
    if (!s.equals("")) {
      ...
      if (!extracted
          && sSplit.length == 2
          && lHelper.isNoun(sSplit[0])
          && sSplit[1].equals("availble")) {
        verb = "be";
      ...
      if (!extracted
          && sSplit.length == 2
          && lHelper.isNoun(sSplit[0])
          && sSplit[1].endsWith("ed")
          && lHelper.isVerb(sSplit[1])) {
      ...
      if (!extracted
          && sSplit.length == 2
          && lHelper.isVerb(sSplit[0])
          && lHelper.isNoun(sSplit[1])) {
        verb = lHelper.getInfinitiveOfAction(sSplit[0]);
      ...
      if (!extracted
          && sSplit.length == 2
          && lHelper.isAdjective(sSplit[0])
          && lHelper.isAdjective(sSplit[1])) {
        verb = "be";
        bo = "";
        modList.put(
      ...
      if (!extracted
        ...
        ModifierRecord mr2 =
            new ModifierRecord(ModifierRecord.TYPE_QUANT, ModifierRecord.TARGET_ROLE);
        modList.put("ANY", mr2);
        extracted = true;
      }
