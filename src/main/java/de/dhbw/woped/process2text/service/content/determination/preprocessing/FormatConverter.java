...
import de.dhbw.woped.process2text.model.process.Activity;
import de.dhbw.woped.process2text.model.process.Element;
...
public class FormatConverter {
    ...
    for (Task t : p.getTasks()) {
      ...
      if (converterMap.containsKey(id)) {
        ...
        if (elem.getClass().toString().endsWith("Event")) {
          de.dhbw.woped.process2text.model.process.Event e =
              (de.dhbw.woped.process2text.model.process.Event) elem;
          pm.addEvent(e);
        ...
        if (g.getGatewayType() == GatewayType.XOR) {
          de.dhbw.woped.process2text.model.process.Gateway gw =
              new de.dhbw.woped.process2text.model.process.Gateway(
                  ...
                  null,
                  null,
                  de.dhbw.woped.process2text.model.process.GatewayType.XOR);
          pm.addGateway(gw);
        ...
        if (g.getGatewayType() == GatewayType.AND) {
          de.dhbw.woped.process2text.model.process.Gateway gw =
              new de.dhbw.woped.process2text.model.process.Gateway(
                  ...
                  null,
                  null,
                  de.dhbw.woped.process2text.model.process.GatewayType.AND);
          pm.addGateway(gw);
          idMap.put(g.getId(), gw.getId());
          elemMap.put(gw.getId(), gw);
        }
        if (g.getGatewayType() == GatewayType.EVENT) {
          de.dhbw.woped.process2text.model.process.Gateway gw =
              new de.dhbw.woped.process2text.model.process.Gateway(
                  ...
                  null,
                  null,
                  de.dhbw.woped.process2text.model.process.GatewayType.EVENT);
          pm.addGateway(gw);
          idMap.put(g.getId(), gw.getId());
          elemMap.put(gw.getId(), gw);
        }
  ...
  public Process transformToRigidFormat(ProcessModel pm) {
    ...
    // Transform events
    for (de.dhbw.woped.process2text.model.process.Event e : pm.getEvents().values()) {
    ...
    // Transform gateway
    for (de.dhbw.woped.process2text.model.process.Gateway g : pm.getGateways().values()) {
      if (g.getType() == de.dhbw.woped.process2text.model.process.GatewayType.XOR) {
        Gateway gt = new Gateway(GatewayType.XOR, Integer.toString(g.getId()));
        elementMap.put(g.getId(), gt);
      }
      if (g.getType() == de.dhbw.woped.process2text.model.process.GatewayType.OR) {
        Gateway gt = new Gateway(GatewayType.OR, Integer.toString(g.getId()));
        elementMap.put(g.getId(), gt);
      }
      if (g.getType() == de.dhbw.woped.process2text.model.process.GatewayType.AND) {
        Gateway gt = new Gateway(GatewayType.AND, Integer.toString(g.getId()));
        elementMap.put(g.getId(), gt);
      }
      if (g.getType() == de.dhbw.woped.process2text.model.process.GatewayType.EVENT) {
    ...
    // Transform arcs
    for (de.dhbw.woped.process2text.model.process.Arc arc : pm.getArcs().values()) {
